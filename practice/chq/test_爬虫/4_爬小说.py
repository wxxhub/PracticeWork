#[1]加载第三方工具
import requests
from bs4 import BeautifulSoup;
import os;
import time;
#小说的网为：
xiaoshuo_url = "https://www.biqukan.com/%s/";



#[4]获取小说每一章节的内容
def Get_xiaoshuo_mulu_context(a_context,a_context_url,header):
    #1.再次进行网络访问:
    requests_02 = requests.get(a_context_url,data=header);
    #2.判断网络是否访问成功
    if requests_02.encoding == 200:
        #3.修改编码格式
        requests_02.encoding ="gbk";
        #4.获取网页源码
        context_data = requests_02.text;
        #5.开始解析源码 -soup
        soup = BeautifulSoup(context_data,"html.parser");
        #6.获取包含小说文字的标签
        xiaoshuo_context = soup.find("div",attrs={"id":"content"}).get_text();
        print("=======================");
        file_xiaoshuo = open("%s.txt"%a_context,"w",1,encoding="utf-8");
        file_xiaoshuo.write(str(xiaoshuo_context).replace("\xa0",""));
        print(xiaoshuo_context)
        print("======================")



        pass
    else:
        print("%s:网络访问失败"%a_context);
        
        pass


    pass


#[3]获取小说目录
def Get_xiaoshuo_mulu(url,xiaoshuo_name,header):
    #1.开始请求网络数据：
    #第一个参数：请求的地址 第二个参数：请求的头文件信息；
    requests_01 = requests.get(url=url,data = header);
    #2.检查是否访问成功
    print("网络请求的返回码：",requests_01.status_code);
    if requests_01.status_code == 200:
        #3.将网站下载到本地中
        print("当前网页的编码格式：",requests_01.encoding);
        #4.修改请求到的数据编码格式
        requests_01.encoding = "";
        #5.请求源码 -data = html.read();
        data = requests_01.text;
        #print(str(data))
        #6.print(str(data))
        file_path = "D:\/Program Files\/feiq\/Recv Files\/08_Request_网络小说爬虫\/01_笔趣看小说爬虫下载工具\/%s\/"%xiaoshuo_name;
        print(file_path)
        #7.判断路径是否存在
        if not os.path.exists(file_path):
             os.makedirs(file_path);
             pass

        #8.存储文件
        file_xiaoshuo = open("%s%s.html"%(file_path,xiaoshuo_name),"w",1);
        file_xiaoshuo.write(data);
        file_xiaoshuo.close();
        print("源码已经保存在本地文件中");
        #9.开始解析数据
        soup = BeautifulSoup(data,"html.parser");
        #9-1获取整个div
        div_listmain = soup.find("div",attrs={"class":"listmain"});
        #9-2获取d1
        d1_01 =div_listmain.d1;
        #print(d1_01)
        #创建一个变量，用于记录什么时候下载
        start_dowm = False;
        i = 1;
        #9-3遍历所有数据
        
        for tab_dd  in d1_01:
            #print("第%d的数据: %s"%(i,tab_dd));
            #9-4:判断参数：

            if tab_dd =="\n":
                #不做任何处理
                pass
            #正文
            elif tab_dd.string.replace(" ","") == "正文":
                start_dowm = True;
                pass
            elif start_dowm:
                #开始下载：
                #间隔时间0.5秒下载一章
                """
                     <dd><a href ="/0_790/15948251.html">第一章 蟒雀吞龙</a></dd>  
                """
                #print("第%d行的数据 : %s" %(i,tab_dd));
                a_context = tab_dd.a.string;
                a_context_url = "https://www.biqukan.com/"+tab_dd.a["href"]
                # 详情地址：https://www.biqukan.com/37_37651/13150974.html
                # /0_790/15948251.html
                print("[%s]下载地址：%s"%(a_context,a_context_url));
                #调用函数下载小说内容：
                #第一个参数：小说每个章节名称 第二个参数：小说的每个章节的地址
                time.sleep(5);
                Get_xiaoshuo_mulu_context(a_context,a_context_url,header);


                pass

            else:
                #最新章节模块才会执行，不做任何处理
                pass

                pass
            i+=1;
            pass
        

        pass
    else:
        print("网络访问失败: 请求码：%d - 请检查网址"%requests_01.status_code);
        pass



    pass



#[2]====创建一个爬虫入口
if __name__ == "__main__":
    print("=====进入程序====");
    print("欢迎使用python小说爬虫");
    print("目前软件支持有限");
    
    #1.请用户输入小说下载编号：
    print("请用户输入小说编码格式为：()");
    xiaoshuo_code = input ("=====请输入小说下载编号=======\n");
    url =xiaoshuo_url%xiaoshuo_code;
    print("小说地址为：",url)
    #2.用户输入小说的名称

    xiaoshuo_name = input("========请输入小说名称========")
    print("小说名称为:",xiaoshuo_name);
    #3.创建网络请求
    #01.创建一个用户代理-用于请求header文件
    """

    """
    header = {"User-Agent":"Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.110 Safari/537.36 SE 2.X MetaSr 1.0"};
    #调用下载目录函数
    # replace(" ",""):用于去除字符串中所有的空格
    # 第一个参数：小说的地址，第二个参数：小说的名称 第三个参数：请求头文件信息
    Get_xiaoshuo_mulu(url,xiaoshuo_name.replace(" ",""),header);

    pass




