...
# 【5】数据的录入
def Play_data_Excel(book_class_list,book_data_lists):
    # 1.创建表格
    wb = Workbook();
    # 2.创建选项卡
    i = 0;
    for book_item_name in book_class_list:
        ws = wb.create_sheet(book_item_name,i);
        i+=1;
        pass
    # 录入数据
    j = 0;
    for item_class_data in book_data_lists:
        ws_01 = wb[book_class_list[j]];
        print("====:",item_class_data);
        # 创建一个标题头：
        ws_01.append(["书籍名称","作者及翻译","时间","出版社","价格","评分","书籍链接","书籍图片"]);
...
def Get_book_item_context(book_class_item,douban_book_url,header):
    ...
    if requests_01.status_code == 200:
        ...
        for tag_dl in tag_dls:
            ...
            print(["《"+book_name+"》", book_author, book_time, book_add, book_jg,book_rating_nums,book_url,book_imgurl])
            book_item_data.append(["《"+str(book_name).strip()+"》"
                                      , str(book_author).strip()
                                      , str(book_time).strip()
                                      , str(book_add).strip()
                                      , str(book_jg).strip()
                                      ,str(book_rating_nums).strip()
                                      ,str(book_url).strip()
                                      ,str(book_imgurl).strip()]);
...
def Do_Spider(book_class_list,douban_book_url,header):
    ...
    for book_class_item in book_class_list:
        ...
        print("*****************************")
        # 4.录入到表格中
        Play_data_Excel(book_class_list,book_data_lists);
